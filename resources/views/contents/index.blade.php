...
<section class="row">
	<div class="col-4 col-sm-3 col-lg-2 pr-0">
		<div class="btn-group-vertical w-100 h-100">
			<a href="{{route('contents')}}" class="btn btn-primary text-white mx-0" style="border-top-right-radius: 0;">All</a>
		...
		<div class="row">
			<div class="col p-3">
				...
				<div class="row justify-content-start mt-4 px-3">
					...
					<a href="{{ route('items', [preg_replace('/\s+/', '_',strtolower($i->item->item_name))]) }}" class="text-center border rounded-lg border-dark content-item text-dark text-decoration-none" style="width: 100px;">
						<img src="{{$i->item->getImage()}}" class="card-img-top img-fluid item-image item-container" draggable='false' style="width: 75px; height: auto;"/>
						<h6 class="item-name">{{$i->item->item_name}}</h6>
					</a>
					@endforeach
				</div>
			</div>
